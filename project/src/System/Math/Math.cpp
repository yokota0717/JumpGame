...
{}
Math::Vec::~Vec() {}
void Math::Vec::Offset(const Math::Vec & v) {
	x += v.x;
	y += v.y;
}
void Math::Vec::Offset(const float x, const float y) {
	this->x += x;
	this->y += y;
}
Math::Vec Math::Vec::OffsetCopy(const Math::Vec& v) {
	return Math::Vec(x + v.x, y + v.y);
}
Math::Vec Math::Vec::OffsetCopy(const float x, const float y) {
	return Math::Vec(this->x + x, this->y + y);
}
void Math::Vec::Zero(){
	x = 0;
	y = 0;
...
Math::Rect::Rect(Vec tl, Vec tr, Vec bl, Vec br)
	...
	br(br)
{}
Math::Rect::Rect(const Vec & topleft, const Vec & bottomright)
	:
	tl(topleft), tr({ bottomright.x, topleft.y }),
	bl({ topleft.x, bottomright.y }), br(bottomright)
{}
...
void Math::Rect::Offset(const float x, const float y) {
	tl.x += x;
	tr.x += x;
	bl.x += x;
	br.x += x;
...
void Math::Rect::Offset(const Vec & v) {
	tl.x += v.x;
	tr.x += v.x;
	bl.x += v.x;
	br.x += v.x;
